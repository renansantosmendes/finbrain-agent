...
O logger `finbrain` também registra, por requisição de `/chat`:
- Qual comando foi selecionado (`command=/price`) ou que a mensagem era livre (`free-form message`).
- Cada tool que o agente chamou nesse turno e com quais argumentos (`agent: ... calling tool=... args=...`), e um preview do resultado (`agent: ... tool_result tool=... preview=...`) — ver `_log_agent_activity` em [app.py](app.py). Só cobre o turno atual (mensagens após o último `HumanMessage`), não o histórico acumulado da sessão.
